...
def test_exact_bucket_id_read_preserves_long_bullets_across_trace_append(mcp_client):
    ...
    assert before[:len(original)] == original
    assert marker_at > before.index(original)
    assert "[exact_bucket_id:true]" in before[len(original):]
    ...
    assert after[:len(appended)] == appended
    assert marker_at > after.index(appended)
...
def test_prompt_injection_text_is_returned_verbatim_body_first(mcp_client):
    ...
    assert marker_at > result.index(content)
    assert "[content_role:stored_memory_data]" not in result
