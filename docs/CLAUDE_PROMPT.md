...
- **`breath_search(query, domain="", max_results=0)`** — 忘事后的唯一搜索路径：
  ...
  - `breath_search(query="完整 bucket_id 或 letter_id")` → 按 ID 直读完整原文，跳过向量、摘要和改写；若长桶提示预算不足，改用 `breath_advanced(query="完整 ID", max_results=1, max_tokens=20000)`。plan / feel 仍用对应读取入口。
  - `breath_search(query="她最近的工作状态", domain="work,relationship")` → 带主题域过滤，逗号分隔。
  - 返回时记忆正文在前、来源与操作 ID 在末尾；不显示 Footprint、权重、importance 数值或相似度。
...
返回里**带 📌 的是我钉的核心准则**；只要没有被 `digested` / `dont_surface` 主动隐藏，它就会保持置顶。`[语义命中]` 是向量召回的定性来源标记，不包含分数；`source:letter_paragraph` 是续接信原文段落，`沉底旧记忆` 只在活跃记忆无答案时出现。

#### `hold(content)` — 我把当下这一件事记下

一句话的事用这个。会自动打标 (domain / valence / arousal / tags / 桶名)，一次调用固定建立一只桶；只有正文完全相同的重试会幂等复用，不会把语义相近的两次经历自动合并。
...
通常返回 `新建→桶名`；正文完全相同的重试会复用已有桶，避免重复写入。
